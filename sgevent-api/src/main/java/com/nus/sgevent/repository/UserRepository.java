...
public interface UserRepository extends CrudRepository<EventUser, Integer> {
	...
	@Query(value = "Update event_user set password=?2,email_address=?3,user_role=?4 where user_name=?1",nativeQuery = true)
	boolean UpdateUser(String UserName,String Password, String EmailAddress, int UserRole);
	
	@Query(value = "Update event_user set password=?2 where user_name=?1",nativeQuery = true)
